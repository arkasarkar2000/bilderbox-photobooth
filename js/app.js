...
const FRAMES = [
  { id: "none", label: "None", thumb: "" },
  {
    id: "frame-classic",
    label: "Classic",
    thumb: "outline:3px solid #f5f0e8;outline-offset:-3px;",
  },
  {
    id: "frame-dark",
    label: "Dark",
    thumb: "outline:3px solid #111;outline-offset:-3px;",
  },
  {
    id: "frame-gold",
    label: "Gold",
    thumb: "outline:3px solid #d4a853;outline-offset:-3px;",
  },
  {
    id: "frame-pink",
    label: "Pink",
    thumb: "outline:3px solid #f4a7c0;outline-offset:-3px;",
  },
  {
    id: "frame-polaroid",
    label: "Polaroid",
    thumb: "outline:3px solid #faf8f2;outline-offset:-3px;",
  },
  {
    id: "frame-film",
    label: "Film",
    thumb: "box-shadow:3px 0 0 #111,-3px 0 0 #111;",
  },
];
...
async function startCamera() {
  ...
  if (!navigator.mediaDevices?.getUserMedia) {
    showCameraError(
      "Your browser does not support camera access. Please use Chrome, Safari, or Firefox.",
    );
    return;
  }
...
function setLayout(id) {
  ...
  document
    ...
    .forEach((b) => b.classList.toggle("active", b.dataset.layoutId === id));
...
async function capturePhoto() {
  ...
  if (photos.filter((p) => p !== null).length >= LAYOUT_CAPS[currentLayout])
    return;
  capturing = true;
...
function grabFrame() {
  ...
  previewCtx.filter = "none";
  paintFrame(previewCtx, w, h, currentFrame);
  return previewCvs.toDataURL(
    "image/jpeg",
    0.95,
  ); /* JPEG for individual shots */
}
...
async function composeStrip(cvs, ctx2, alive, layout, withTimestamp) {
  ...
  if (layout === "strip4") {
    ...
    const iw = 320,
      ih = 320,
      cols = 2,
      rows = Math.ceil(capped.length / 2);
    ...
    const iw = 240,
      ...
      polBot = 44;
    const cols = 2,
      rows = Math.ceil(capped.length / cols),
      gutter = 28;
    cvs.width = cols * (iw + polPad * 2) + (cols + 1) * gutter + 40;
    cvs.height =
      labelH +
      rows * (ih + polPad + polBot) +
      (rows - 1) * gutter +
      50 +
      footerH;
...
async function downloadStrip() {
  ...
  try {
    ...
    if (tsPrev) {
      ...
      tsPrev.style.width = "100%";
      tsPrev.style.height = "auto";
    }
...
async function shareStripFromPreview() {
  if (!pendingStripDataUrl) return;
  if (!navigator.share) {
    triggerDownload(pendingStripDataUrl, "bilderbox-strip.png");
    return;
  }
  try {
...
async function buildSharePreview() {
  ...
  try {
    if (shareFormat === "reel") {
      ...
      ctx2.font = '10px "DM Mono", monospace';
      ctx2.textAlign = "center";
      ctx2.fillText(new Date().toLocaleDateString("en-GB"), sw / 2, sh - 8);
...
async function showQR() {
  ...
  qrWrap.innerHTML =
    "<div style=\"color:var(--text3);font-size:0.7rem;font-family:'DM Mono',monospace;padding:1rem;\">Generating QR…</div>";
  try {
    const previewCanvas = document.getElementById("share-preview-canvas");
    const qrCvs = document.createElement("canvas");
    const scale = Math.min(
      1,
      300 / Math.max(previewCanvas.width, previewCanvas.height),
    );
    qrCvs.width = Math.round(previewCanvas.width * scale);
    qrCvs.height = Math.round(previewCanvas.height * scale);
    qrCvs
      .getContext("2d")
      .drawImage(previewCanvas, 0, 0, qrCvs.width, qrCvs.height);
