...
const FRAMES = [
  ...
  { id: 'frame-classic',  label: 'Classic',  thumb: 'outline:4px solid #f5f0e8;outline-offset:-4px;' },
  { id: 'frame-dark',     label: 'Dark',     thumb: 'outline:4px solid #111;outline-offset:-4px;' },
  { id: 'frame-gold',     label: 'Gold',     thumb: 'outline:4px solid #d4a853;outline-offset:-4px;' },
  { id: 'frame-pink',     label: 'Pink',     thumb: 'outline:4px solid #f4a7c0;outline-offset:-4px;' },
  { id: 'frame-polaroid', label: 'Polaroid', thumb: 'outline:4px solid #faf8f2;outline-offset:-4px;margin-bottom:8px;' },
  { id: 'frame-film',     label: 'Film',     thumb: 'box-shadow:4px 0 0 #111,-4px 0 0 #111;' },
]
...
function grabFrame() {
  ...
  return previewCvs.toDataURL('image/jpeg', 0.92)
}
...
function paintFrame(c, w, h, frame) {
  ...
  if (frame === 'frame-dark') {
    c.strokeStyle = '#0a0a0a'; c.lineWidth = 24; c.strokeRect(12,12,w-24,h-24)
  }
  if (frame === 'frame-gold') {
    const g = c.createLinearGradient(0,0,w,h)
    g.addColorStop(0,'#8b6914'); g.addColorStop(0.35,'#d4a853')
    g.addColorStop(0.65,'#f0c96a'); g.addColorStop(1,'#8b6914')
  ...
  if (frame === 'frame-polaroid') {
    c.fillStyle = '#faf8f2'
    c.fillRect(0,0,w,16); c.fillRect(0,0,16,h)
    c.fillRect(w-16,0,16,h); c.fillRect(0,h-56,w,56)
  }
  if (frame === 'frame-film') {
    c.fillStyle = '#111'
    c.fillRect(0,0,18,h); c.fillRect(w-18,0,18,h)
    c.fillStyle = '#222'
...
function composeStrip(cvs, ctx2, alive, layout, forShare) {
  ...
  if (layout === 'strip4' || alive.length < 3) {
    /* vertical strip */
    const iw = 420, ih = Math.round(420 * 3 / 4)
    const totalH = labelH + pad * 2 + alive.length * ih + (alive.length - 1) * gap
    cvs.width = iw + pad * 2; cvs.height = totalH
    ...
    /* 2x2 instagram grid */
    const iw = 300, ih = 300
    const cols = 2, rows = Math.ceil(alive.length / 2)
    ...
    const ih = 260, iw = Math.round(260 * 4 / 3)
    const sprocketH = 28
    ...
    const iw = 240, ih = 240, polPad = 14, polBot = 48
    const cols = 2
    const rows  = Math.ceil(alive.length / cols)
    const gutter = 30
    cvs.width  = cols * (iw + polPad * 2) + (cols + 1) * gutter + 60
    cvs.height = labelH + rows * (ih + polPad + polBot) + (rows - 1) * gutter + 60
...
function refreshStripPreview() {
  ;['strip-preview-canvas-desktop', 'strip-preview-canvas-mobile'].forEach(id => {
    ...
    if (alive.length === 0) {
      ...
      ctx2.fillStyle = '#1e1525'; ctx2.fillRect(0,0,w,h)
      ctx2.fillStyle = '#5e5070'
...
function downloadStrip() {
  ...
  setTimeout(() => {
    triggerDownload(stripCvs.toDataURL('image/jpeg', 0.95), 'bilderbox-strip-' + Date.now() + '.jpg')
...
function buildSharePreview() {
  ...
  if (shareFormat === 'reel') {
    ...
    const iw = sw - 40, ih = Math.round(iw * 3/4)
    ...
    const pad = 8
    const cols = 2, iw = (sz - pad * 3) / 2, ih = iw
    alive.slice(0,4).forEach((p, i) => {
      const col = i % 2, row = Math.floor(i / 2)
      const img = new Image(); img.src = p.dataUrl
      const s = Math.min(img.naturalWidth || 1280, img.naturalHeight || 960)
...
async function downloadShareImage() {
  const cvs = document.getElementById('share-preview-canvas')
  setTimeout(() => {
    triggerDownload(cvs.toDataURL('image/jpeg', 0.95), 'bilderbox-' + Date.now() + '.jpg')
    showToast('Saved!')
  }, 200)
}
