...
window.addEventListener('unhandledrejection', e => {
  ...
  /* don't surface share cancellations or benign aborts */
...
function checkBrowserCompat() {
  ...
  if (issues.length) {
    ...
    if (warn && msg) {
      msg.textContent = issues.join(' · ') + '. Try Chrome or Safari.'
      warn.style.display = 'flex'
    }
    /* disable capture buttons */
    ;['capture-btn','burst-btn'].forEach(id => {
      const btn = document.getElementById(id)
      if (btn) { btn.disabled = true; btn.title = issues[0] }
    })
  }
}
checkBrowserCompat()
...
const FRAMES = [
  ...
  { id: 'frame-classic',  label: 'Classic',  thumb: 'outline:3px solid #f5f0e8;outline-offset:-3px;' },
  { id: 'frame-dark',     label: 'Dark',     thumb: 'outline:3px solid #111;outline-offset:-3px;' },
  { id: 'frame-gold',     label: 'Gold',     thumb: 'outline:3px solid #d4a853;outline-offset:-3px;' },
  { id: 'frame-pink',     label: 'Pink',     thumb: 'outline:3px solid #f4a7c0;outline-offset:-3px;' },
  { id: 'frame-polaroid', label: 'Polaroid', thumb: 'outline:3px solid #faf8f2;outline-offset:-3px;' },
  { id: 'frame-film',     label: 'Film',     thumb: 'box-shadow:3px 0 0 #111,-3px 0 0 #111;' },
]
...
/* ── Camera — with full error handling ── */
async function startCamera() {
  /* hide error state, show loading */
  document.getElementById('camera-error').style.display = 'none'
  document.getElementById('startup').style.display      = 'flex'

  if (!navigator.mediaDevices?.getUserMedia) {
    showCameraError('Your browser does not support camera access. Please use Chrome, Safari, or Firefox.')
    return
  }

  /* must be HTTPS or localhost */
  if (location.protocol !== 'https:' && location.hostname !== 'localhost') {
    showCameraError('Camera requires a secure connection (HTTPS). Please open this page over HTTPS.')
    return
  }
  ...
  try {
    ...
    if (['OverconstrainedError','NotReadableError','NotFoundError','AbortError'].includes(err.name)) {
      try {
        ...
        showCameraError(friendlyCameraError(fallbackErr))
        return
      }
    } else {
      showCameraError(friendlyCameraError(err))
      return
    }
  }
...
function friendlyCameraError(err) {
  switch (err.name) {
    ...
    case 'PermissionDeniedError':
      return 'Camera access was denied. Click "How to fix this" below for instructions.'
    case 'NotFoundError':
    case 'DevicesNotFoundError':
      return 'No camera found on this device.'
    case 'NotReadableError':
    case 'TrackStartError':
      return 'Camera is in use by another app. Close other video apps and try again.'
    case 'OverconstrainedError':
      return 'Camera does not meet the required settings. Try again.'
    case 'SecurityError':
      return 'Camera blocked for security reasons. Make sure you\'re on HTTPS.'
    default:
...
function showCameraHelp() {
  document.getElementById('camera-help-modal').classList.add('open')
}
...
async function startBurst() {
  ...
  try {
    if (timerSecs > 0) await countdown(timerSecs)
    const slots = Math.min(4, cap - alive.length)
    for (let i = 0; i < slots; i++) {
      doFlash()
      addPhoto(grabFrame())
      if (i < slots - 1) await sleep(720)
    }
    await sleep(180)
  } catch (err) {
    showToast('Burst failed — please try again')
...
/* ── Grab frame
   - Individual photos: JPEG 95% (good quality, much smaller file)
   - Strip download: PNG via composeStrip (lossless final output)
── */
function grabFrame() {
  ...
  previewCtx.save()
  previewCtx.scale(-1, 1)                       /* unmirrored capture */
  previewCtx.drawImage(video, -w, 0, w, h)
  previewCtx.restore()
  previewCtx.filter = 'none'
  paintFrame(previewCtx, w, h, currentFrame)
  return previewCvs.toDataURL('image/jpeg', 0.95)  /* JPEG for individual shots */
}
...
/* ── Strip composition — fully async, PNG output ── */
async function composeStrip(cvs, ctx2, alive, layout, withTimestamp) {
  ...
  if (layout === 'strip4') {
    ...
    for (let i = 0; i < stripPhotos.length; i++) {
      const img = await loadImg(stripPhotos[i].dataUrl)
    ...
    for (let i = 0; i < capped.length; i++) {
      ...
      ctx2.drawImage(img, sx, sy, s, s, gap + col*(iw+gap), labelH + gap + row*(ih+gap), iw, ih)
    }
    if (withTimestamp) drawStripFooter(ctx2, cvs.width, cvs.height, footerH)
    ...
    for (let x = 12; x < cvs.width; x += 24) {
      ctx2.fillRect(x, 6, 12, 16)
      ctx2.fillRect(x, cvs.height - footerH - 22, 12, 16)
    }
    for (let i = 0; i < capped.length; i++) {
      const img = await loadImg(capped[i].dataUrl)
      ctx2.drawImage(img, gap + i * (iw + gap), sprocketH, iw, ih)
    }
    ctx2.fillStyle = '#f4a7c0'
    ctx2.font = 'italic 11px "Playfair Display", serif'
    ctx2.textAlign = 'left'
    ctx2.fillText('bilderbox', 10, cvs.height - footerH - 6)
    if (withTimestamp) {
      ctx2.fillStyle = '#5e5070'; ctx2.font = '10px "DM Mono", monospace'
      ctx2.textAlign = 'right'
      ctx2.fillText(new Date().toLocaleDateString('en-GB'), cvs.width - 10, cvs.height - 8)
    }

  } else if (layout === 'polaroid') {
    const capped = alive.slice(0, 6)
    const iw = 240, ih = 240, polPad = 12, polBot = 44
    ...
    for (let i = 0; i < capped.length; i++) {
      ...
      ctx2.save()
      ctx2.translate(cx, cy); ctx2.rotate(rot)
      ctx2.shadowColor = 'rgba(0,0,0,0.5)'; ctx2.shadowBlur = 16
      ctx2.fillStyle = '#faf8f2'
      ctx2.beginPath(); roundRect(ctx2, -cardW/2, -cardH/2, cardW, cardH, 3); ctx2.fill()
      ctx2.shadowBlur = 0
...
function drawStripHeader(ctx2, w, labelH) {
  ctx2.fillStyle = '#f4a7c0'
  ctx2.font = 'italic bold 20px "Playfair Display", serif'
...
/* ── Download strip — timestamp prompt flow ── */
async function downloadStrip() {
  ...
  try {
    ...
    if (tsPrev) {
      tsPrev.width  = stripCvs.width
      tsPrev.height = stripCvs.height
      tsPrev.getContext('2d').drawImage(stripCvs, 0, 0)
      tsPrev.style.width = '100%'; tsPrev.style.height = 'auto'
    }
    document.getElementById('timestamp-modal').classList.add('open')
  } catch (err) {
    showToast('Could not generate strip — please try again')
...
async function finalizeDownload(withTimestamp) {
  ...
  if (withTimestamp) {
    try {
      await composeStrip(stripCvs, stripCtx, alive, currentLayout, true)
      pendingStripDataUrl = stripCvs.toDataURL('image/png')
    } catch (err) {
      showToast('Could not add timestamp — saving without it')
      console.error('Timestamp render error:', err)
    }
  }
...
async function shareStripFromPreview() {
  ...
  try {
    ...
    const file = new File([blob], 'bilderbox-strip.png', { type: 'image/png' })
    await navigator.share({ files: [file], title: 'bilderbox photobooth' })
  } catch (err) {
    if (!err.message?.toLowerCase().includes('cancel')) showToast('Share failed')
  }
}
...
function buildStripItem(dataUrl, idx) {
  ...
  item.className = 'strip-item'; item.dataset.idx = idx
  const img = document.createElement('img')
  img.src = dataUrl; img.alt = 'Photo ' + (idx + 1); img.loading = 'lazy'
  const actions = document.createElement('div')
  actions.className = 'strip-actions'
  ;[
    { label: 'View',   fn: () => openPhotoModal(idx) },
    { label: 'Save',   fn: () => downloadPhotoIdx(idx) },
    { label: 'Delete', fn: () => deletePhoto(idx), danger: true },
  ].forEach(b => {
    const btn = document.createElement('button')
    btn.className = 'strip-action-btn'; btn.textContent = b.label
...
async function buildSharePreview() {
  ...
  try {
    if (shareFormat === 'reel') {
      ...
      ctx2.textAlign = 'center'; ctx2.fillText('bilderbox', sw/2, 44)
      const iw = sw - 40, ih = Math.round(iw * 4 / 3)
      const sliceCount = Math.min(alive.length, 4)
      const totalImgH  = sliceCount * (ih + 10)
      const startY     = (sh - totalImgH) / 2
      for (let i = 0; i < sliceCount; i++) {
        const img = await loadImg(alive[i].dataUrl)
        ctx2.drawImage(img, 20, startY + i * (ih + 10), iw, ih)
      }
      ctx2.fillStyle = '#5e5070'; ctx2.font = '10px "DM Mono", monospace'
      ctx2.textAlign = 'center'; ctx2.fillText(new Date().toLocaleDateString('en-GB'), sw/2, sh - 8)

    } else if (shareFormat === 'grid') {
      const sz = 480, pad = 6
      cvs.width = sz; cvs.height = sz
      ctx2.fillStyle = '#0e0b10'; ctx2.fillRect(0,0,sz,sz)
      const iw = (sz - pad * 3) / 2, ih = iw
      for (let i = 0; i < Math.min(alive.length, 4); i++) {
        const img = await loadImg(alive[i].dataUrl)
        const col = i % 2, row = Math.floor(i / 2)
        const s = Math.min(img.naturalWidth, img.naturalHeight)
        const sx = (img.naturalWidth - s) / 2, sy = (img.naturalHeight - s) / 2
        ctx2.drawImage(img, sx, sy, s, s, pad + col*(iw+pad), pad + row*(ih+pad), iw, ih)
      }
    } else {
      await composeStrip(cvs, ctx2, alive, currentLayout, false)
    }
    cvs.style.width = '100%'; cvs.style.height = 'auto'
  } catch (err) {
    console.error('Share preview error:', err)
    showToast('Preview failed — try again')
  }
}

async function shareToInstagram() {
  await downloadShareImage()
  showToast('Saved — open Instagram and post from your camera roll!')
}
...
async function shareNative() {
  ...
  try {
    ...
    const file = new File([blob], 'bilderbox.png', { type: 'image/png' })
    await navigator.share({ files: [file], title: 'bilderbox photobooth' })
  } catch (err) {
    if (!err.message?.toLowerCase().includes('cancel')) showToast('Share failed — try downloading instead')
  }
}
...
async function downloadShareImage() {
  const cvs = document.getElementById('share-preview-canvas')
  try {
    triggerDownload(cvs.toDataURL('image/png'), 'bilderbox-' + Date.now() + '.png')
    showToast('Saved!')
  } catch {
...
async function showQR() {
  ...
  const alive     = photos.filter(p => p !== null)
  if (!alive.length) { showToast('No photos yet'); return }
  qrSection.style.display = 'flex'
  qrWrap.innerHTML = '<div style="color:var(--text3);font-size:0.7rem;font-family:\'DM Mono\',monospace;padding:1rem;">Generating QR…</div>'
  try {
    ...
    qrCvs.width  = Math.round(previewCanvas.width  * scale)
    qrCvs.height = Math.round(previewCanvas.height * scale)
    ...
    qrWrap.innerHTML = ''
    if (window.QRCode) {
      new QRCode(qrWrap, { text: encoded, width: 180, height: 180, colorDark: '#f4a7c0', colorLight: '#0e0b10', correctLevel: QRCode.CorrectLevel.L })
    } else {
      qrWrap.textContent = 'QR library not loaded'
    }
  } catch (err) {
    qrWrap.textContent = 'QR generation failed'
...
function deleteCurrent() {
  if (currentShareIdx === null) return
  deletePhoto(currentShareIdx); closeModal('photo-modal')
}
...
function triggerDownload(dataUrl, filename) {
  ...
  } catch { showToast('Download failed — try right-clicking to save') }
}

function canvasToBlob(cvs) {
  return new Promise((resolve, reject) => {
    try { cvs.toBlob(blob => blob ? resolve(blob) : reject(new Error('Canvas toBlob failed')), 'image/png') }
    catch (e) { reject(e) }
  })
}
